...
<body>
        ...
        <main>
            <div class="container-fluid row">
                    <form class="mx-auto px-4 py-3 order-md-1 order-1" action="{{ route('animation_store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="col-md-6 col-12">
                                <div class="form-group">
                                    ...
                                    <select class="form-control" name="scenes_id" id="scenes_id">
                                        ...
                                        <option {{ ( old("scenes_id") == $items->id )? "selected":""  }} value="{{$items->id}}">{{$items->nama}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="form-group">
                                    <label for="description">Description</label>
                                    <textarea type="text" class="form-control" id="description" name="description" rows="8" cols="50">{{ old('description')? old('description') : '' }}</textarea>
                                    @error('description')
